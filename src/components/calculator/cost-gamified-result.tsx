...
import { deriveCostDashboardMetrics } from "@/lib/cost-dashboard";
...
export function CostGamifiedResult({
  ...
  const metrics = hasResult
    ? deriveCostDashboardMetrics(calculatorId, label, value, unit, detail)
    : null;
  ...
  return (
    <GamifiedDashboardFrame
      accent="cost"
      label={label}
      ambientClassName="bg-emerald-500/[0.08] dark:bg-amber-500/[0.12]"
      className={className}
    >
      <div
        key={resultKey}
      ...
      >
        {!hasResult ? (
          ...
          <div className="grid gap-8 sm:grid-cols-[minmax(160px,200px)_1fr] sm:items-center sm:gap-6">
            <CostVisual
              needlePercent={metrics.needlePercent}
              tier={metrics.tier}
            ...
            <div className="flex min-w-0 flex-col gap-4">
              <div
                className={cn(
                  ...
                  metrics.tier === "standard" &&
                    "[--neon-from:#f59e0b] [--neon-to:#eab308]",
                  metrics.tier === "hog" &&
