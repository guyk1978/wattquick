export type {
  CalculatorCategory,
  CalculatorComputeFn,
  CalculatorDefinition,
  CalculatorFieldDef,
  CalculatorFieldInputType,
  CalculatorFieldOption,
  CalculatorId,
  CalculatorMeta,
  CalculatorResultConfig,
  CalculatorResultDisplay,
  CalculatorSeoContent,
  CalculatorSeoSection,
  CalculatorContentSection,
} from "./types";
export {
  calculators,
  CALCULATOR_SLUGS,
  type CalculatorSlug,
} from "@/data/calculators";
export {
  CALCULATOR_CATEGORY_DESCRIPTIONS,
  CALCULATOR_CATEGORY_LABELS,
  toMeta,
} from "./types";
export {
  CALCULATOR_ORDER,
  getAllCalculatorDefinitions,
  getAllCalculatorMeta,
  getCalculatorDefinition,
  getCalculatorMeta,
  getCalculatorsByCategory,
  getSuggestions,
} from "./registry";
export { getRelatedCalculators } from "./related";
export { isCalculatorId } from "./utils";
export { isCalculatorCategory } from "./categories";
export {
  CALCULATOR_ID_TO_GUIDE_SLUG,
  CALCULATOR_TOOL_SLUGS,
  GUIDE_LANDING_SLUGS,
  TOOLS_CALCULATORS_SLUGS,
  getAllGuideLandings,
  getCalculatorIdForToolSlug,
  getGuideLanding,
  getGuideLinkLabelForCalculator,
  getGuidePathForCalculator,
  getGuideLandingsByCategory,
  isCalculatorToolSlug,
  isGuideLandingSlug,
  isToolsCalculatorsSlug,
  type CalculatorToolSlug,
  type GuideLandingSlug,
  type ToolsCalculatorsSlug,
} from "./calculator-landings-registry";
export type {
  GuideLandingContent,
  GuideLandingDefinition,
  GuideLandingFooterResource,
  GuideLandingFaq,
} from "./landing-types";
export {
  getCalculatorToolHref,
  getGuideLandingHref,
  getResourceGuideHref,
} from "./landing-types";
export {
  BATTERY_PERCENTAGE_FOOTER_RESOURCES,
  BATTERY_PERCENTAGE_GUIDE_HREF,
  BATTERY_PERCENTAGE_LANDING_SLUG,
  BATTERY_PERCENTAGE_LANDING_SLUGS,
  BATTERY_PERCENTAGE_TOOL_HREF,
  getAllBatteryPercentageLandings,
  getBatteryPercentageLanding,
  isBatteryPercentageLandingSlug,
  type BatteryPercentageGuideDefinition,
  type BatteryPercentageLandingSlug,
} from "./batterypercentageLandings";
export {
  EV_SOC_FOOTER_RESOURCES,
  EV_SOC_GUIDE_HREF,
  EV_SOC_LANDING_SLUG,
  EV_SOC_LANDING_SLUGS,
  getAllEvSocLandings,
  getEvSocLanding,
  isEvSocLandingSlug,
  type EvSocGuideDefinition,
  type EvSocLandingSlug,
} from "./evSocLandings";
export {
  REMAINING_BATTERY_CAPACITY_GUIDE_HREF,
  REMAINING_BATTERY_CAPACITY_LANDING_SLUG,
  REMAINING_BATTERY_CAPACITY_LANDING_SLUGS,
  getAllRemainingBatteryCapacityLandings,
  getRemainingBatteryCapacityLanding,
  isRemainingBatteryCapacityLandingSlug,
  type RemainingBatteryCapacityGuideDefinition,
  type RemainingBatteryCapacityLandingSlug,
} from "./remainingBatteryCapacityLandings";
export {
  ELECTRIC_SCOOTER_RANGE_CALCULATOR_GUIDE_HREF,
  ELECTRIC_SCOOTER_RANGE_CALCULATOR_LANDING_SLUG,
  HOW_FAR_CAN_MY_ESCOOTER_GO_GUIDE_HREF,
  HOW_FAR_CAN_MY_ESCOOTER_GO_LANDING_SLUG,
  ESCOOTER_BATTERY_RANGE_ESTIMATOR_GUIDE_HREF,
  ESCOOTER_BATTERY_RANGE_ESTIMATOR_LANDING_SLUG,
  ESCOOTER_RANGE_CALCULATOR_ID,
  ESCOOTER_RANGE_FOOTER_RESOURCES,
  ESCOOTER_RANGE_LANDING_SLUGS,
  ESCOOTER_RANGE_TOOL_HREF,
  ESCOOTER_RANGE_TOOL_PATH,
  getAllEscooterRangeLandings,
  getEscooterRangeLanding,
  getEscooterRangeToolFooterLinks,
  isEscooterRangeLandingSlug,
  type EscooterRangeGuideDefinition,
  type EscooterRangeLandingSlug,
} from "./escooterRangeLandings";
export {
  ESCOOTER_TIRE_PRESSURE_CALCULATOR_ID,
  ESCOOTER_TIRE_PRESSURE_FOOTER_RESOURCES,
  ESCOOTER_TIRE_PRESSURE_LANDING_SLUGS,
  ESCOOTER_TIRE_PRESSURE_TOOL_HREF,
  ESCOOTER_TIRE_PRESSURE_TOOL_PATH,
  getAllEscooterTirePressureLandings,
  getEscooterTirePressureLanding,
  getEscooterTirePressureToolFooterLinks,
  isEscooterTirePressureLandingSlug,
  type EscooterTirePressureLandingSlug,
} from "./escooterTirePressureLandings";
export {
  ESCOOTER_MAX_SPEED_CALCULATOR_ID,
  ESCOOTER_MAX_SPEED_FOOTER_RESOURCES,
  ESCOOTER_MAX_SPEED_LANDING_SLUGS,
  ESCOOTER_MAX_SPEED_TOOL_HREF,
  ESCOOTER_MAX_SPEED_TOOL_PATH,
  ESCOOTER_TOP_SPEED_CALCULATOR_GUIDE_HREF,
  ESCOOTER_TOP_SPEED_CALCULATOR_LANDING_SLUG,
  CALCULATE_MAX_SPEED_FROM_MOTOR_KV_AND_VOLTAGE_GUIDE_HREF,
  CALCULATE_MAX_SPEED_FROM_MOTOR_KV_AND_VOLTAGE_LANDING_SLUG,
  THEORETICAL_ESCOOTER_SPEED_ESTIMATOR_GUIDE_HREF,
  THEORETICAL_ESCOOTER_SPEED_ESTIMATOR_LANDING_SLUG,
  getAllEscooterMaxSpeedLandings,
  getEscooterMaxSpeedLanding,
  getEscooterMaxSpeedToolFooterLinks,
  isEscooterMaxSpeedLandingSlug,
  type EscooterMaxSpeedGuideDefinition,
  type EscooterMaxSpeedLandingSlug,
} from "./escooterMaxSpeedLandings";
export {
  ESCOOTER_HILL_CLIMB_CALCULATOR_ID,
  ESCOOTER_HILL_CLIMB_FOOTER_RESOURCES,
  ESCOOTER_HILL_CLIMB_GRADE_CALCULATOR_GUIDE_HREF,
  ESCOOTER_HILL_CLIMB_GRADE_CALCULATOR_LANDING_SLUG,
  CALCULATE_ESCOOTER_GRADEABILITY_UNDER_LOAD_AND_VOLTAGE_SAG_GUIDE_HREF,
  CALCULATE_ESCOOTER_GRADEABILITY_UNDER_LOAD_AND_VOLTAGE_SAG_LANDING_SLUG,
  ELECTRIC_SCOOTER_TORQUE_AND_HILL_GRADE_ESTIMATOR_GUIDE_HREF,
  ELECTRIC_SCOOTER_TORQUE_AND_HILL_GRADE_ESTIMATOR_LANDING_SLUG,
  ESCOOTER_HILL_CLIMB_LANDING_SLUGS,
  ESCOOTER_HILL_CLIMB_TOOL_HREF,
  ESCOOTER_HILL_CLIMB_TOOL_PATH,
  getAllEscooterHillClimbLandings,
  getEscooterHillClimbLanding,
  getEscooterHillClimbToolFooterLinks,
  isEscooterHillClimbLandingSlug,
  type EscooterHillClimbGuideDefinition,
  type EscooterHillClimbLandingSlug,
} from "./escooterHillClimbLandings";
export {
  ESCOOTER_TIRE_WEAR_CALCULATOR_ID,
  ESCOOTER_TIRE_WEAR_FOOTER_RESOURCES,
  ESCOOTER_TIRE_WEAR_LANDING_SLUGS,
  ESCOOTER_TYRE_WEAR_LIFE_CALCULATOR_GUIDE_HREF,
  ESCOOTER_TYRE_WEAR_LIFE_CALCULATOR_LANDING_SLUG,
  ESTIMATE_ESCOOTER_TYRE_LIFE_IN_KILOMETERS_GUIDE_HREF,
  ESTIMATE_ESCOOTER_TYRE_LIFE_IN_KILOMETERS_LANDING_SLUG,
  CALCULATE_TYRE_LIFESPAN_BASED_ON_RIDING_SURFACE_GUIDE_HREF,
  CALCULATE_TYRE_LIFESPAN_BASED_ON_RIDING_SURFACE_LANDING_SLUG,
  ESCOOTER_TIRE_WEAR_TOOL_HREF,
  ESCOOTER_TIRE_WEAR_TOOL_PATH,
  getAllEscooterTireWearLandings,
  getEscooterTireWearLanding,
  getEscooterTireWearToolFooterLinks,
  isEscooterTireWearLandingSlug,
  type EscooterTireWearGuideDefinition,
  type EscooterTireWearLandingSlug,
} from "./escooterTireWearLandings";
export {
  ESCOOTER_CHARGE_TIME_CALCULATOR_ID,
  ESCOOTER_CHARGE_TIME_CALCULATOR_GUIDE_HREF,
  ESCOOTER_CHARGE_TIME_CALCULATOR_LANDING_SLUG,
  COMPARE_ESCOOTER_CHARGER_SPEEDS_2A_3A_4A_GUIDE_HREF,
  COMPARE_ESCOOTER_CHARGER_SPEEDS_2A_3A_4A_LANDING_SLUG,
  BATTERY_CHARGE_TIME_ESTIMATOR_FOR_36V_48V_PACKS_GUIDE_HREF,
  BATTERY_CHARGE_TIME_ESTIMATOR_FOR_36V_48V_PACKS_LANDING_SLUG,
  ESCOOTER_CHARGE_TIME_FOOTER_RESOURCES,
  ESCOOTER_CHARGE_TIME_LANDING_SLUGS,
  ESCOOTER_CHARGE_TIME_TOOL_HREF,
  ESCOOTER_CHARGE_TIME_TOOL_PATH,
  getAllEscooterChargeTimeLandings,
  getEscooterChargeTimeLanding,
  getEscooterChargeTimeToolFooterLinks,
  isEscooterChargeTimeLandingSlug,
  type EscooterChargeTimeGuideDefinition,
  type EscooterChargeTimeLandingSlug,
} from "./escooterChargeTimeLandings";
export {
  ESCOOTER_COST_PER_KM_CALCULATOR_ID,
  ESCOOTER_COST_PER_KM_FOOTER_RESOURCES,
  ESCOOTER_COST_PER_KM_LANDING_SLUGS,
  ESCOOTER_COST_PER_KILOMETER_CALCULATOR_GUIDE_HREF,
  ESCOOTER_COST_PER_KILOMETER_CALCULATOR_LANDING_SLUG,
  ESCOOTER_VS_PUBLIC_TRANSIT_COST_COMPARISON_GUIDE_HREF,
  ESCOOTER_VS_PUBLIC_TRANSIT_COST_COMPARISON_LANDING_SLUG,
  ELECTRIC_SCOOTER_COMMUTE_SAVINGS_CALCULATOR_GUIDE_HREF,
  ELECTRIC_SCOOTER_COMMUTE_SAVINGS_CALCULATOR_LANDING_SLUG,
  ESCOOTER_COST_PER_KM_TOOL_HREF,
  ESCOOTER_COST_PER_KM_TOOL_PATH,
  getAllEscooterCostPerKmLandings,
  getEscooterCostPerKmLanding,
  getEscooterCostPerKmToolFooterLinks,
  isEscooterCostPerKmLandingSlug,
  type EscooterCostPerKmGuideDefinition,
  type EscooterCostPerKmLandingSlug,
} from "./escooterCostPerKmLandings";
export {
  ESCOOTER_WEIGHT_LIMIT_CALCULATOR_ID,
  ESCOOTER_WEIGHT_LIMIT_FOOTER_RESOURCES,
  ESCOOTER_WEIGHT_LIMIT_LANDING_SLUGS,
  ESCOOTER_RIDER_WEIGHT_LIMIT_CALCULATOR_GUIDE_HREF,
  ESCOOTER_RIDER_WEIGHT_LIMIT_CALCULATOR_LANDING_SLUG,
  CALCULATE_ESCOOTER_DECK_AND_MOTOR_STRESS_GUIDE_HREF,
  CALCULATE_ESCOOTER_DECK_AND_MOTOR_STRESS_LANDING_SLUG,
  ESCOOTER_PAYLOAD_CAPACITY_AND_MOTOR_LOAD_ESTIMATOR_GUIDE_HREF,
  ESCOOTER_PAYLOAD_CAPACITY_AND_MOTOR_LOAD_ESTIMATOR_LANDING_SLUG,
  ESCOOTER_WEIGHT_LIMIT_TOOL_HREF,
  ESCOOTER_WEIGHT_LIMIT_TOOL_PATH,
  getAllEscooterWeightLimitLandings,
  getEscooterWeightLimitLanding,
  getEscooterWeightLimitToolFooterLinks,
  isEscooterWeightLimitLandingSlug,
  type EscooterWeightLimitGuideDefinition,
  type EscooterWeightLimitLandingSlug,
} from "./escooterWeightLimitLandings";
export {
  BATTERY_C_RATING_AND_CONTROLLER_DISCHARGE_MATCH_GUIDE_HREF,
  BATTERY_C_RATING_AND_CONTROLLER_DISCHARGE_MATCH_LANDING_SLUG,
  CALCULATE_MAX_DISCHARGE_AMPS_FOR_BATTERY_SAFETY_GUIDE_HREF,
  CALCULATE_MAX_DISCHARGE_AMPS_FOR_BATTERY_SAFETY_LANDING_SLUG,
  ESCOOTER_PEAK_AMPS_CALCULATOR_ID,
  ESCOOTER_PEAK_AMPS_FOOTER_RESOURCES,
  ESCOOTER_PEAK_AMPS_LANDING_SLUGS,
  ESCOOTER_PEAK_DISCHARGE_CURRENT_CALCULATOR_GUIDE_HREF,
  ESCOOTER_PEAK_DISCHARGE_CURRENT_CALCULATOR_LANDING_SLUG,
  ESCOOTER_PEAK_AMPS_TOOL_HREF,
  ESCOOTER_PEAK_AMPS_TOOL_PATH,
  getAllEscooterPeakAmpsLandings,
  getEscooterPeakAmpsLanding,
  getEscooterPeakAmpsToolFooterLinks,
  isEscooterPeakAmpsLandingSlug,
  type EscooterPeakAmpsGuideDefinition,
  type EscooterPeakAmpsLandingSlug,
} from "./escooterPeakAmpsLandings";
export {
  PREDICT_ESCOOTER_BRAKE_PAD_LONGEVITY_GUIDE_HREF,
  PREDICT_ESCOOTER_BRAKE_PAD_LONGEVITY_LANDING_SLUG,
  CALCULATE_BRAKE_PAD_WEAR_FROM_REGEN_SETTINGS_GUIDE_HREF,
  CALCULATE_BRAKE_PAD_WEAR_FROM_REGEN_SETTINGS_LANDING_SLUG,
  ESCOOTER_BRAKE_PAD_LIFE_CALCULATOR_GUIDE_HREF,
  ESCOOTER_BRAKE_PAD_LIFE_CALCULATOR_LANDING_SLUG,
  ESCOOTER_BRAKE_PAD_WEAR_CALCULATOR_ID,
  ESCOOTER_BRAKE_PAD_WEAR_FOOTER_RESOURCES,
  ESCOOTER_BRAKE_PAD_WEAR_LANDING_SLUGS,
  ESCOOTER_BRAKE_PAD_WEAR_TOOL_HREF,
  ESCOOTER_BRAKE_PAD_WEAR_TOOL_PATH,
  getAllEscooterBrakePadWearLandings,
  getEscooterBrakePadWearLanding,
  getEscooterBrakePadWearToolFooterLinks,
  isEscooterBrakePadWearLandingSlug,
  type EscooterBrakePadWearGuideDefinition,
  type EscooterBrakePadWearLandingSlug,
} from "./escooterBrakePadWearLandings";
export {
  XT30_VS_XT60_VS_XT90_POWER_HANDLING_ESTIMATOR_GUIDE_HREF,
  XT30_VS_XT60_VS_XT90_POWER_HANDLING_ESTIMATOR_LANDING_SLUG,
  CALCULATE_CONNECTOR_HEAT_AND_VOLTAGE_DROP_I2R_GUIDE_HREF,
  CALCULATE_CONNECTOR_HEAT_AND_VOLTAGE_DROP_I2R_LANDING_SLUG,
  ESCOOTER_CONNECTOR_LOSS_CALCULATOR_ID,
  ESCOOTER_CONNECTOR_LOSS_FOOTER_RESOURCES,
  ESCOOTER_CONNECTOR_LOSS_LANDING_SLUGS,
  ESCOOTER_CONNECTOR_POWER_LOSS_CALCULATOR_GUIDE_HREF,
  ESCOOTER_CONNECTOR_POWER_LOSS_CALCULATOR_LANDING_SLUG,
  ESCOOTER_CONNECTOR_LOSS_TOOL_HREF,
  ESCOOTER_CONNECTOR_LOSS_TOOL_PATH,
  getAllEscooterConnectorLossLandings,
  getEscooterConnectorLossLanding,
  getEscooterConnectorLossToolFooterLinks,
  isEscooterConnectorLossLandingSlug,
  type EscooterConnectorLossGuideDefinition,
  type EscooterConnectorLossLandingSlug,
} from "./escooterConnectorLossLandings";
export {
  CALCULATE_ESCOOTER_MAINTENANCE_BASED_ON_MILEAGE_GUIDE_HREF,
  CALCULATE_ESCOOTER_MAINTENANCE_BASED_ON_MILEAGE_LANDING_SLUG,
  ELECTRIC_SCOOTER_SERVICE_INTERVAL_TRACKER_GUIDE_HREF,
  ELECTRIC_SCOOTER_SERVICE_INTERVAL_TRACKER_LANDING_SLUG,
  ESCOOTER_MAINTENANCE_SCHEDULE_CALCULATOR_GUIDE_HREF,
  ESCOOTER_MAINTENANCE_SCHEDULE_CALCULATOR_LANDING_SLUG,
  ESCOOTER_MAINTENANCE_SCHEDULE_CALCULATOR_ID,
  ESCOOTER_MAINTENANCE_SCHEDULE_FOOTER_RESOURCES,
  ESCOOTER_MAINTENANCE_SCHEDULE_LANDING_SLUGS,
  ESCOOTER_MAINTENANCE_SCHEDULE_TOOL_HREF,
  ESCOOTER_MAINTENANCE_SCHEDULE_TOOL_PATH,
  getAllEscooterMaintenanceScheduleLandings,
  getEscooterMaintenanceScheduleLanding,
  getEscooterMaintenanceScheduleToolFooterLinks,
  isEscooterMaintenanceScheduleLandingSlug,
  type EscooterMaintenanceScheduleGuideDefinition,
  type EscooterMaintenanceScheduleLandingSlug,
} from "./escooterMaintenanceScheduleLandings";
export {
  ELECTRIC_TRUCK_RANGE_AND_PAYLOAD_CALCULATOR_GUIDE_HREF,
  ELECTRIC_TRUCK_RANGE_AND_PAYLOAD_CALCULATOR_LANDING_SLUG,
  IMPACT_OF_PAYLOAD_ON_EV_TRUCK_RANGE_GUIDE_HREF,
  IMPACT_OF_PAYLOAD_ON_EV_TRUCK_RANGE_LANDING_SLUG,
  EV_HEAVY_DUTY_TRUCK_EFFICIENCY_CALCULATOR_GUIDE_HREF,
  EV_HEAVY_DUTY_TRUCK_EFFICIENCY_CALCULATOR_LANDING_SLUG,
  EV_TRUCK_RANGE_CALCULATOR_ID,
  EV_TRUCK_RANGE_FOOTER_RESOURCES,
  EV_TRUCK_RANGE_LANDING_SLUGS,
  EV_TRUCK_RANGE_TOOL_HREF,
  EV_TRUCK_RANGE_TOOL_PATH,
  getAllEvTruckRangeLandings,
  getEvTruckRangeLanding,
  getEvTruckRangeToolFooterLinks,
  isEvTruckRangeLandingSlug,
  type EvTruckRangeGuideDefinition,
  type EvTruckRangeLandingSlug,
} from "./evTruckRangeLandings";
export {
  UPS_RUNTIME_CALCULATOR_GUIDE_HREF,
  UPS_RUNTIME_CALCULATOR_LANDING_SLUG,
  BATTERY_BACKUP_TIME_ESTIMATOR_GUIDE_HREF,
  BATTERY_BACKUP_TIME_ESTIMATOR_LANDING_SLUG,
  ESTIMATE_UPS_BATTERY_CAPACITY_BASED_ON_LOAD_GUIDE_HREF,
  ESTIMATE_UPS_BATTERY_CAPACITY_BASED_ON_LOAD_LANDING_SLUG,
  UPS_RUNTIME_CALCULATOR_ID,
  UPS_RUNTIME_FOOTER_RESOURCES,
  UPS_RUNTIME_LANDING_SLUGS,
  UPS_RUNTIME_TOOL_HREF,
  UPS_RUNTIME_TOOL_PATH,
  getAllUpsRuntimeLandings,
  getUpsRuntimeLanding,
  getUpsRuntimeToolFooterLinks,
  isUpsRuntimeLandingSlug,
  type UpsRuntimeGuideDefinition,
  type UpsRuntimeLandingSlug,
} from "./upsRuntimeLandings";
export {
  BATTERY_BANK_SIZE_CALCULATOR_GUIDE_HREF,
  BATTERY_BANK_SIZE_CALCULATOR_LANDING_SLUG,
  AMP_HOURS_NEEDED_FOR_BATTERY_BANK_GUIDE_HREF,
  AMP_HOURS_NEEDED_FOR_BATTERY_BANK_LANDING_SLUG,
  OFF_GRID_BATTERY_CAPACITY_ESTIMATOR_GUIDE_HREF,
  OFF_GRID_BATTERY_CAPACITY_ESTIMATOR_LANDING_SLUG,
  BATTERY_BANK_SIZE_CALCULATOR_ID,
  BATTERY_BANK_SIZE_FOOTER_RESOURCES,
  BATTERY_BANK_SIZE_LANDING_SLUGS,
  BATTERY_BANK_SIZE_TOOL_HREF,
  BATTERY_BANK_SIZE_TOOL_PATH,
  getAllBatteryBankSizeLandings,
  getBatteryBankSizeLanding,
  getBatteryBankSizeToolFooterLinks,
  isBatteryBankSizeLandingSlug,
  type BatteryBankSizeGuideDefinition,
  type BatteryBankSizeLandingSlug,
} from "./batteryBankSizeLandings";
export {
  CALCULATE_E_BIKE_DISTANCE_PER_CHARGE_GUIDE_HREF,
  CALCULATE_E_BIKE_DISTANCE_PER_CHARGE_LANDING_SLUG,
  ELECTRIC_BIKE_BATTERY_RANGE_CALCULATOR_GUIDE_HREF,
  ELECTRIC_BIKE_BATTERY_RANGE_CALCULATOR_LANDING_SLUG,
  EBIKE_RANGE_ESTIMATOR_GUIDE_HREF,
  EBIKE_RANGE_ESTIMATOR_LANDING_SLUG,
  EBIKE_RANGE_ESTIMATOR_CALCULATOR_ID,
  EBIKE_RANGE_ESTIMATOR_FOOTER_RESOURCES,
  EBIKE_RANGE_ESTIMATOR_LANDING_SLUGS,
  EBIKE_RANGE_ESTIMATOR_TOOL_HREF,
  EBIKE_RANGE_ESTIMATOR_TOOL_PATH,
  getAllEbikeRangeEstimatorLandings,
  getEbikeRangeEstimatorLanding,
  getEbikeRangeEstimatorToolFooterLinks,
  isEbikeRangeEstimatorLandingSlug,
  type EbikeRangeEstimatorGuideDefinition,
  type EbikeRangeEstimatorLandingSlug,
} from "./ebikeRangeLandings";
export {
  CALCULATE_SOLAR_PANEL_AND_BATTERY_CAPACITY_FOR_OFF_GRID_LOADS_GUIDE_HREF,
  CALCULATE_SOLAR_PANEL_AND_BATTERY_CAPACITY_FOR_OFF_GRID_LOADS_LANDING_SLUG,
  OFF_GRID_SOLAR_SYSTEM_SIZING_CALCULATOR_GUIDE_HREF,
  OFF_GRID_SOLAR_SYSTEM_SIZING_CALCULATOR_LANDING_SLUG,
  TWELVE_V_TWENTY_FOUR_V_SOLAR_POWER_SYSTEM_ESTIMATOR_GUIDE_HREF,
  TWELVE_V_TWENTY_FOUR_V_SOLAR_POWER_SYSTEM_ESTIMATOR_LANDING_SLUG,
  RV_SOLAR_CALCULATOR_CALCULATOR_ID,
  RV_SOLAR_CALCULATOR_FOOTER_RESOURCES,
  RV_SOLAR_CALCULATOR_LANDING_SLUGS,
  RV_SOLAR_CALCULATOR_TOOL_HREF,
  RV_SOLAR_CALCULATOR_TOOL_PATH,
  getAllRvSolarCalculatorLandings,
  getRvSolarCalculatorLanding,
  getRvSolarCalculatorToolFooterLinks,
  isRvSolarCalculatorLandingSlug,
  type RvSolarCalculatorGuideDefinition,
  type RvSolarCalculatorLandingSlug,
} from "./rvSolarCalculatorLandings";
export {
  APPLIANCE_MONTHLY_ENERGY_CALCULATOR_ID,
  APPLIANCE_MONTHLY_ENERGY_CONSUMPTION_CALCULATOR_GUIDE_HREF,
  APPLIANCE_MONTHLY_ENERGY_CONSUMPTION_CALCULATOR_LANDING_SLUG,
  CALCULATE_ELECTRICITY_COST_PER_APPLIANCE_GUIDE_HREF,
  CALCULATE_ELECTRICITY_COST_PER_APPLIANCE_LANDING_SLUG,
  CONVERT_APPLIANCE_WATTS_TO_MONTHLY_KWH_GUIDE_HREF,
  CONVERT_APPLIANCE_WATTS_TO_MONTHLY_KWH_LANDING_SLUG,
  APPLIANCE_MONTHLY_ENERGY_FOOTER_RESOURCES,
  APPLIANCE_MONTHLY_ENERGY_LANDING_SLUGS,
  APPLIANCE_MONTHLY_ENERGY_TOOL_HREF,
  APPLIANCE_MONTHLY_ENERGY_TOOL_PATH,
  getAllApplianceMonthlyEnergyLandings,
  getApplianceMonthlyEnergyLanding,
  getApplianceMonthlyEnergyToolFooterLinks,
  isApplianceMonthlyEnergyLandingSlug,
  type ApplianceMonthlyEnergyGuideDefinition,
  type ApplianceMonthlyEnergyLandingSlug,
} from "./applianceMonthlyEnergyLandings";
export {
  BUILDING_ENVELOPE_HEAT_LOSS_CALCULATOR_GUIDE_HREF,
  BUILDING_ENVELOPE_HEAT_LOSS_CALCULATOR_LANDING_SLUG,
  ESTIMATE_HEAT_LOSS_BY_R_VALUE_GUIDE_HREF,
  ESTIMATE_HEAT_LOSS_BY_R_VALUE_LANDING_SLUG,
  HEAT_LOSS_CALCULATION_CALCULATOR_GUIDE_HREF,
  HEAT_LOSS_CALCULATION_CALCULATOR_LANDING_SLUG,
  HEAT_LOSS_INSULATION_CALCULATOR_ID,
  HEAT_LOSS_INSULATION_FOOTER_RESOURCES,
  HEAT_LOSS_INSULATION_LANDING_SLUGS,
  HEAT_LOSS_INSULATION_TOOL_HREF,
  HEAT_LOSS_INSULATION_TOOL_PATH,
  getAllHeatLossInsulationLandings,
  getHeatLossInsulationLanding,
  getHeatLossInsulationToolFooterLinks,
  isHeatLossInsulationLandingSlug,
  type HeatLossInsulationGuideDefinition,
  type HeatLossInsulationLandingSlug,
} from "./heatLossInsulationLandings";
export {
  ENERGY_SAVINGS_FROM_WALL_AND_WINDOW_INSULATION_GUIDE_HREF,
  ENERGY_SAVINGS_FROM_WALL_AND_WINDOW_INSULATION_LANDING_SLUG,
  ESTIMATE_ANNUAL_HEATING_AND_COOLING_SAVINGS_GUIDE_HREF,
  ESTIMATE_ANNUAL_HEATING_AND_COOLING_SAVINGS_LANDING_SLUG,
  HOME_INSULATION_SAVINGS_CALCULATOR_GUIDE_HREF,
  HOME_INSULATION_SAVINGS_CALCULATOR_LANDING_SLUG,
  HOME_INSULATION_SAVINGS_CALCULATOR_ID,
  HOME_INSULATION_SAVINGS_FOOTER_RESOURCES,
  HOME_INSULATION_SAVINGS_LANDING_SLUGS,
  HOME_INSULATION_SAVINGS_TOOL_HREF,
  HOME_INSULATION_SAVINGS_TOOL_PATH,
  getAllHomeInsulationSavingsLandings,
  getHomeInsulationSavingsLanding,
  getHomeInsulationSavingsToolFooterLinks,
  isHomeInsulationSavingsLandingSlug,
  type HomeInsulationSavingsGuideDefinition,
  type HomeInsulationSavingsLandingSlug,
} from "./homeInsulationSavingsLandings";
export {
  LED_LIGHTING_SAVINGS_AND_CO2_CALCULATOR_GUIDE_HREF,
  LED_LIGHTING_SAVINGS_AND_CO2_CALCULATOR_LANDING_SLUG,
  COMPARE_LED_VS_INCANDESCENT_BULB_COSTS_GUIDE_HREF,
  COMPARE_LED_VS_INCANDESCENT_BULB_COSTS_LANDING_SLUG,
  LED_ROI_CALCULATOR_GUIDE_HREF,
  LED_ROI_CALCULATOR_LANDING_SLUG,
  LED_SAVINGS_ROI_CALCULATOR_ID,
  LED_SAVINGS_ROI_FOOTER_RESOURCES,
  LED_SAVINGS_ROI_LANDING_SLUGS,
  LED_SAVINGS_ROI_TOOL_HREF,
  LED_SAVINGS_ROI_TOOL_PATH,
  getAllLedSavingsRoiLandings,
  getLedSavingsRoiLanding,
  getLedSavingsRoiToolFooterLinks,
  isLedSavingsRoiLandingSlug,
  type LedSavingsRoiGuideDefinition,
  type LedSavingsRoiLandingSlug,
} from "./ledSavingsRoiLandings";
export {
  POOL_ENERGY_THERMAL_COVER_CALCULATOR_ID,
  POOL_ENERGY_THERMAL_COVER_FOOTER_RESOURCES,
  POOL_ENERGY_THERMAL_COVER_LANDING_SLUGS,
  POOL_ENERGY_THERMAL_COVER_TOOL_HREF,
  POOL_ENERGY_THERMAL_COVER_TOOL_PATH,
  getAllPoolEnergyThermalCoverLandings,
  getPoolEnergyThermalCoverLanding,
  getPoolEnergyThermalCoverToolFooterLinks,
  isPoolEnergyThermalCoverLandingSlug,
  type PoolEnergyThermalCoverLandingSlug,
} from "./poolEnergyThermalCoverLandings";
export {
  TOU_SHIFTING_SAVINGS_CALCULATOR_ID,
  TOU_SHIFTING_SAVINGS_FOOTER_RESOURCES,
  TOU_SHIFTING_SAVINGS_LANDING_SLUGS,
  TOU_SHIFTING_SAVINGS_TOOL_HREF,
  TOU_SHIFTING_SAVINGS_TOOL_PATH,
  getAllTouShiftingSavingsLandings,
  getTouShiftingSavingsLanding,
  getTouShiftingSavingsToolFooterLinks,
  isTouShiftingSavingsLandingSlug,
  type TouShiftingSavingsLandingSlug,
} from "./touShiftingSavingsLandings";
export {
  DEMAND_CHARGE_CALCULATOR_ID,
  DEMAND_CHARGE_CALCULATOR_FOOTER_RESOURCES,
  DEMAND_CHARGE_CALCULATOR_LANDING_SLUGS,
  DEMAND_CHARGE_CALCULATOR_TOOL_HREF,
  DEMAND_CHARGE_CALCULATOR_TOOL_PATH,
  getAllDemandChargeCalculatorLandings,
  getDemandChargeCalculatorLanding,
  getDemandChargeCalculatorToolFooterLinks,
  isDemandChargeCalculatorLandingSlug,
  type DemandChargeCalculatorLandingSlug,
} from "./demandChargeCalculatorLandings";
export {
  BATTERY_COST_CALCULATOR_ID,
  BATTERY_COST_FOOTER_RESOURCES,
  BATTERY_COST_LANDING_SLUGS,
  BATTERY_COST_TOOL_HREF,
  BATTERY_COST_TOOL_PATH,
  getAllBatteryCostLandings,
  getBatteryCostLanding,
  getBatteryCostToolFooterLinks,
  isBatteryCostLandingSlug,
  type BatteryCostLandingSlug,
} from "./batteryCostLandings";
export {
  ELECTRICITY_BILL_CALCULATOR_ID,
  ELECTRICITY_BILL_FOOTER_RESOURCES,
  ELECTRICITY_BILL_LANDING_SLUGS,
  ELECTRICITY_BILL_TOOL_HREF,
  ELECTRICITY_BILL_TOOL_PATH,
  getAllElectricityBillLandings,
  getElectricityBillLanding,
  getElectricityBillToolFooterLinks,
  isElectricityBillLandingSlug,
  type ElectricityBillLandingSlug,
} from "./electricityBillLandings";
export {
  AH_TO_WH_CALCULATOR_ID,
  AH_TO_WH_FOOTER_RESOURCES,
  AH_TO_WH_LANDING_SLUGS,
  AH_TO_WH_TOOL_HREF,
  AH_TO_WH_TOOL_PATH,
  getAllAhToWhLandings,
  getAhToWhLanding,
  getAhToWhToolFooterLinks,
  isAhToWhLandingSlug,
  type AhToWhLandingSlug,
} from "./ahToWhLandings";
export {
  WH_TO_AH_CALCULATOR_ID,
  WH_TO_AH_FOOTER_RESOURCES,
  WH_TO_AH_LANDING_SLUGS,
  WH_TO_AH_TOOL_HREF,
  WH_TO_AH_TOOL_PATH,
  getAllWhToAhLandings,
  getWhToAhLanding,
  getWhToAhToolFooterLinks,
  isWhToAhLandingSlug,
  type WhToAhLandingSlug,
} from "./whToAhLandings";
export {
  KVA_TO_KW_CALCULATOR_ID,
  KVA_TO_KW_FOOTER_RESOURCES,
  KVA_TO_KW_LANDING_SLUGS,
  KVA_TO_KW_TOOL_HREF,
  KVA_TO_KW_TOOL_PATH,
  getAllKvaToKwLandings,
  getKvaToKwLanding,
  getKvaToKwToolFooterLinks,
  isKvaToKwLandingSlug,
  type KvaToKwLandingSlug,
} from "./kvaToKwLandings";
export {
  KW_TO_HP_CALCULATOR_ID,
  KW_TO_HP_FOOTER_RESOURCES,
  KW_TO_HP_LANDING_SLUGS,
  KW_TO_HP_TOOL_HREF,
  KW_TO_HP_TOOL_PATH,
  getAllKwToHpLandings,
  getKwToHpLanding,
  getKwToHpToolFooterLinks,
  isKwToHpLandingSlug,
  type KwToHpLandingSlug,
} from "./kwToHpLandings";
export {
  CONDUCTOR_RESISTANCE_TEMPERATURE_CALCULATOR_ID,
  CONDUCTOR_RESISTANCE_TEMPERATURE_FOOTER_RESOURCES,
  CONDUCTOR_RESISTANCE_TEMPERATURE_LANDING_SLUGS,
  CONDUCTOR_RESISTANCE_TEMPERATURE_TOOL_HREF,
  CONDUCTOR_RESISTANCE_TEMPERATURE_TOOL_PATH,
  getAllConductorResistanceTemperatureLandings,
  getConductorResistanceTemperatureLanding,
  getConductorResistanceTemperatureToolFooterLinks,
  isConductorResistanceTemperatureLandingSlug,
  type ConductorResistanceTemperatureLandingSlug,
} from "./conductorResistanceTemperatureLandings";
export {
  REACTIVE_POWER_CALCULATOR_CALCULATOR_ID,
  REACTIVE_POWER_CALCULATOR_FOOTER_RESOURCES,
  REACTIVE_POWER_CALCULATOR_LANDING_SLUGS,
  REACTIVE_POWER_CALCULATOR_TOOL_HREF,
  REACTIVE_POWER_CALCULATOR_TOOL_PATH,
  getAllReactivePowerCalculatorLandings,
  getReactivePowerCalculatorLanding,
  getReactivePowerCalculatorToolFooterLinks,
  isReactivePowerCalculatorLandingSlug,
  type ReactivePowerCalculatorLandingSlug,
} from "./reactivePowerCalculatorLandings";
export {
  BATTERY_DOD_ENERGY_YIELD_CALCULATOR_ID,
  BATTERY_DOD_ENERGY_YIELD_FOOTER_RESOURCES,
  BATTERY_DOD_ENERGY_YIELD_LANDING_SLUGS,
  BATTERY_DOD_ENERGY_YIELD_TOOL_HREF,
  BATTERY_DOD_ENERGY_YIELD_TOOL_PATH,
  getAllBatteryDodEnergyYieldLandings,
  getBatteryDodEnergyYieldLanding,
  getBatteryDodEnergyYieldToolFooterLinks,
  isBatteryDodEnergyYieldLandingSlug,
  type BatteryDodEnergyYieldLandingSlug,
} from "./batteryDodEnergyYieldLandings";
export {
  BATTERY_RUNTIME_CALCULATOR_ID,
  BATTERY_RUNTIME_FOOTER_RESOURCES,
  BATTERY_RUNTIME_LANDING_SLUGS,
  BATTERY_RUNTIME_TOOL_HREF,
  BATTERY_RUNTIME_TOOL_PATH,
  getAllBatteryRuntimeLandings,
  getBatteryRuntimeLanding,
  getBatteryRuntimeToolFooterLinks,
  isBatteryRuntimeLandingSlug,
  type BatteryRuntimeLandingSlug,
} from "./batteryRuntimeLandings";
export {
  BATTERY_CHARGING_TIME_CALCULATOR_ID,
  BATTERY_CHARGING_TIME_FOOTER_RESOURCES,
  BATTERY_CHARGING_TIME_LANDING_SLUGS,
  BATTERY_CHARGING_TIME_TOOL_HREF,
  BATTERY_CHARGING_TIME_TOOL_PATH,
  getAllBatteryChargingTimeLandings,
  getBatteryChargingTimeLanding,
  getBatteryChargingTimeToolFooterLinks,
  isBatteryChargingTimeLandingSlug,
  type BatteryChargingTimeLandingSlug,
} from "./batteryChargingTimeLandings";
export {
  BATTERY_ENERGY_CALCULATOR_ID,
  BATTERY_ENERGY_FOOTER_RESOURCES,
  BATTERY_ENERGY_LANDING_SLUGS,
  BATTERY_ENERGY_TOOL_HREF,
  BATTERY_ENERGY_TOOL_PATH,
  getAllBatteryEnergyLandings,
  getBatteryEnergyLanding,
  getBatteryEnergyToolFooterLinks,
  isBatteryEnergyLandingSlug,
  type BatteryEnergyLandingSlug,
} from "./batteryEnergyLandings";
export {
  BATTERY_DEPTH_OF_DISCHARGE_CALCULATOR_ID,
  BATTERY_DEPTH_OF_DISCHARGE_FOOTER_RESOURCES,
  BATTERY_DEPTH_OF_DISCHARGE_LANDING_SLUGS,
  BATTERY_DEPTH_OF_DISCHARGE_TOOL_HREF,
  BATTERY_DEPTH_OF_DISCHARGE_TOOL_PATH,
  getAllBatteryDepthOfDischargeLandings,
  getBatteryDepthOfDischargeLanding,
  getBatteryDepthOfDischargeToolFooterLinks,
  isBatteryDepthOfDischargeLandingSlug,
  type BatteryDepthOfDischargeLandingSlug,
} from "./batteryDepthOfDischargeLandings";
export {
  BATTERY_EFFICIENCY_CALCULATOR_ID,
  BATTERY_EFFICIENCY_FOOTER_RESOURCES,
  BATTERY_EFFICIENCY_LANDING_SLUGS,
  BATTERY_EFFICIENCY_TOOL_HREF,
  BATTERY_EFFICIENCY_TOOL_PATH,
  getAllBatteryEfficiencyLandings,
  getBatteryEfficiencyLanding,
  getBatteryEfficiencyToolFooterLinks,
  isBatteryEfficiencyLandingSlug,
  type BatteryEfficiencyLandingSlug,
} from "./batteryEfficiencyLandings";
export {
  BATTERY_SERIES_PARALLEL_CALCULATOR_ID,
  BATTERY_SERIES_PARALLEL_FOOTER_RESOURCES,
  BATTERY_SERIES_PARALLEL_LANDING_SLUGS,
  BATTERY_SERIES_PARALLEL_TOOL_HREF,
  BATTERY_SERIES_PARALLEL_TOOL_PATH,
  getAllBatterySeriesParallelLandings,
  getBatterySeriesParallelLanding,
  getBatterySeriesParallelToolFooterLinks,
  isBatterySeriesParallelLandingSlug,
  type BatterySeriesParallelLandingSlug,
} from "./batterySeriesParallelLandings";
export {
  INVERTER_SIZING_CALCULATOR_ID,
  INVERTER_SIZING_FOOTER_RESOURCES,
  INVERTER_SIZING_LANDING_SLUGS,
  INVERTER_SIZING_TOOL_HREF,
  INVERTER_SIZING_TOOL_PATH,
  getAllInverterSizingLandings,
  getInverterSizingLanding,
  getInverterSizingToolFooterLinks,
  isInverterSizingLandingSlug,
  type InverterSizingLandingSlug,
} from "./inverterSizingLandings";
export {
  DC_CABLE_SIZE_CALCULATOR_ID,
  DC_CABLE_SIZE_FOOTER_RESOURCES,
  DC_CABLE_SIZE_LANDING_SLUGS,
  DC_CABLE_SIZE_TOOL_HREF,
  DC_CABLE_SIZE_TOOL_PATH,
  getAllDcCableSizeLandings,
  getDcCableSizeLanding,
  getDcCableSizeToolFooterLinks,
  isDcCableSizeLandingSlug,
  type DcCableSizeLandingSlug,
} from "./dcCableSizeLandings";
export {
  ESTIMATE_MONTHLY_AND_ANNUAL_ELECTRICITY_COSTS_GUIDE_HREF,
  ESTIMATE_MONTHLY_AND_ANNUAL_ELECTRICITY_COSTS_LANDING_SLUG,
  CALCULATE_HOME_ELECTRICITY_USAGE_BY_CATEGORY_GUIDE_HREF,
  CALCULATE_HOME_ELECTRICITY_USAGE_BY_CATEGORY_LANDING_SLUG,
  WHOLE_HOUSE_ENERGY_BUDGET_CALCULATOR_GUIDE_HREF,
  WHOLE_HOUSE_ENERGY_BUDGET_CALCULATOR_LANDING_SLUG,
  WHOLE_HOUSE_ENERGY_BUDGET_CALCULATOR_ID,
  WHOLE_HOUSE_ENERGY_BUDGET_FOOTER_RESOURCES,
  WHOLE_HOUSE_ENERGY_BUDGET_LANDING_SLUGS,
  WHOLE_HOUSE_ENERGY_BUDGET_TOOL_HREF,
  WHOLE_HOUSE_ENERGY_BUDGET_TOOL_PATH,
  getAllWholeHouseEnergyBudgetLandings,
  getWholeHouseEnergyBudgetLanding,
  getWholeHouseEnergyBudgetToolFooterLinks,
  isWholeHouseEnergyBudgetLandingSlug,
  type WholeHouseEnergyBudgetGuideDefinition,
  type WholeHouseEnergyBudgetLandingSlug,
} from "./wholeHouseEnergyBudgetLandings";
export {
  HOME_EV_CHARGING_COST_ESTIMATOR_GUIDE_HREF,
  HOME_EV_CHARGING_COST_ESTIMATOR_LANDING_SLUG,
  COST_TO_CHARGE_ELECTRIC_CAR_AT_HOME_GUIDE_HREF,
  COST_TO_CHARGE_ELECTRIC_CAR_AT_HOME_LANDING_SLUG,
  EV_CHARGING_COST_CALCULATOR_GUIDE_HREF,
  EV_CHARGING_COST_CALCULATOR_LANDING_SLUG,
  EV_CHARGING_COST_CALCULATOR_ID,
  EV_CHARGING_COST_FOOTER_RESOURCES,
  EV_CHARGING_COST_LANDING_SLUGS,
  EV_CHARGING_COST_TOOL_HREF,
  EV_CHARGING_COST_TOOL_PATH,
  getAllEvChargingCostLandings,
  getEvChargingCostLanding,
  getEvChargingCostToolFooterLinks,
  isEvChargingCostLandingSlug,
  type EvChargingCostGuideDefinition,
  type EvChargingCostLandingSlug,
} from "./evChargingCostLandings";
export {
  CALCULATE_SOLAR_CAPACITY_FROM_DAILY_ENERGY_USAGE_GUIDE_HREF,
  CALCULATE_SOLAR_CAPACITY_FROM_DAILY_ENERGY_USAGE_LANDING_SLUG,
  ESTIMATE_SOLAR_PANEL_WATTAGE_NEEDED_GUIDE_HREF,
  ESTIMATE_SOLAR_PANEL_WATTAGE_NEEDED_LANDING_SLUG,
  SOLAR_PANEL_SIZE_CALCULATOR_GUIDE_HREF,
  SOLAR_PANEL_SIZE_CALCULATOR_LANDING_SLUG,
  SOLAR_PANEL_SIZE_CALCULATOR_ID,
  SOLAR_PANEL_SIZE_FOOTER_RESOURCES,
  SOLAR_PANEL_SIZE_LANDING_SLUGS,
  SOLAR_PANEL_SIZE_TOOL_HREF,
  SOLAR_PANEL_SIZE_TOOL_PATH,
  getAllSolarPanelSizeLandings,
  getSolarPanelSizeLanding,
  getSolarPanelSizeToolFooterLinks,
  isSolarPanelSizeLandingSlug,
  type SolarPanelSizeGuideDefinition,
  type SolarPanelSizeLandingSlug,
} from "./solarPanelSizeLandings";
export {
  WATTS_TO_AMPS_CALCULATOR_GUIDE_HREF,
  WATTS_TO_AMPS_CALCULATOR_LANDING_SLUG,
  CALCULATE_AMPS_FROM_WATTS_AND_VOLTAGE_GUIDE_HREF,
  CALCULATE_AMPS_FROM_WATTS_AND_VOLTAGE_LANDING_SLUG,
  CONVERT_WATTS_TO_AMPS_FOR_ELECTRICAL_LOAD_GUIDE_HREF,
  CONVERT_WATTS_TO_AMPS_FOR_ELECTRICAL_LOAD_LANDING_SLUG,
  WATTS_TO_AMPS_CALCULATOR_ID,
  WATTS_TO_AMPS_FOOTER_RESOURCES,
  WATTS_TO_AMPS_LANDING_SLUGS,
  WATTS_TO_AMPS_TOOL_HREF,
  WATTS_TO_AMPS_TOOL_PATH,
  getAllWattsToAmpsLandings,
  getWattsToAmpsLanding,
  getWattsToAmpsToolFooterLinks,
  isWattsToAmpsLandingSlug,
  type WattsToAmpsGuideDefinition,
  type WattsToAmpsLandingSlug,
} from "./wattsToAmpsLandings";
export {
  getCalculatorHref,
  getCalculatorIdFromToolPath,
} from "../calculator-routes";
export {
  CALCULATOR_USE_CASES,
  getCalculatorUseCase,
  getCalculatorsForUseCase,
  getSortedUseCaseCalculatorIds,
  getUseCaseById,
  isCalculatorUseCase,
  type CalculatorUseCase,
  type CalculatorUseCaseId,
} from "../calculator-use-cases";

import { getAllCalculatorMeta as _getAllCalculatorMeta } from "./registry";

/** @deprecated Use `getAllCalculatorMeta()` */
export const CALCULATOR_LIST = _getAllCalculatorMeta();

/** @deprecated Use `getCalculatorMeta` */
export { getCalculatorMeta as getCalculator } from "./registry";
